...
export class UIController {
  ...
  updateSessionStatus(isSpeaking) {
    ...
    if (isSpeaking === null) {
      ...
      sessionStatus.innerHTML =
        bgHtml +
        '<span style="position: relative; z-index: 1;"><span class="emoji-large">😶</span> AI助手离线中</span>';
      sessionStatus.className = 'status offline';
    } else if (isSpeaking) {
      // 说话中
      sessionStatus.innerHTML =
        bgHtml +
        '<span style="position: relative; z-index: 1;"><span class="emoji-large">😶</span> AI助手说话中</span>';
      sessionStatus.className = 'status speaking';
    } else {
      // 聆听中
      sessionStatus.innerHTML =
        bgHtml +
        '<span style="position: relative; z-index: 1;"><span class="emoji-large">😶</span> AI助手聆听中</span>';
  ...
  updateSessionEmotion(emoji) {
    ...
    sessionStatus.innerHTML =
      bgHtml +
      `<span style="position: relative; z-index: 1;"><span class="emoji-large">${emoji}</span> ${currentText}</span>`;
  }
    ...
    if (sessionStatus && sessionStatus.classList.contains('speaking') && sessionStatusBg) {
      if (stats.pendingPlay > 0) {
        ...
        if (stats.pendingPlay < 5) {
          // 缓冲不足：橙红色半透明
          sessionStatusBg.style.background =
            'linear-gradient(90deg, rgba(255, 152, 0, 0.25), rgba(255, 87, 34, 0.25))';
        } else if (stats.pendingPlay < 10) {
          // 一般：黄绿色半透明
          sessionStatusBg.style.background =
            'linear-gradient(90deg, rgba(205, 220, 57, 0.25), rgba(76, 175, 80, 0.25))';
        } else {
          // 充足：绿蓝色半透明
          sessionStatusBg.style.background =
            'linear-gradient(90deg, rgba(76, 175, 80, 0.25), rgba(33, 150, 243, 0.25))';
        }
      } else {
        // 没有缓冲，隐藏背景
