...
function getLangFiles(fileList: Record<string, any>, messages: Record<string, any>) {
 ...
 * @returns 标准化后的语言代码,如果没有找到匹配则返回 'en'
 */
function normalizeLocale(locale: string, availableLocales: string[]): string {
  // 先检查该语言代码是否直接存在
  ...
  if (normalizedLocale && availableLocales.includes(normalizedLocale)) {
...
function getDefaultLocale(): string {
  try {
    ...
    // 标准化浏览器语言代码
    return normalizeLocale(navigator.language, availableLocales)
  } catch {
    return 'en'
  }
}
...
/**
 * 全局翻译函数 - Vue I18n 官方推荐方式
 * 用于在非组件上下文中使用（如 Pinia store、普通 TS 文件）
 *
 * 注意：在 Vue 组件中，优先使用 useI18n() 或 $t()
 *
 * @param key 翻译键，支持点号路径，如 'keyPage.Mode0'
 * @returns 翻译后的文本
 *
 * @example
 * // 在组件外使用
 * import { t } from '@/wired/share/lang/index'
